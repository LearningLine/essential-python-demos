...
def print_board(game, x_val, o_val):
    for row in game:
        for c in row:
            print('x' if c == x_val else ('o' if c == o_val else '.'), end=' ')
        print()
    print()
...
def is_invalid_play(game, row, col, e, x, o):
...
while not hasWon:
    ...
    row, col = get_played_cell()

    if is_invalid_play(game, row, col, e, x, o):
